...
const EduVerseFirebaseChat: React.FC = () => {
  // User state
    ...
    const initAuth = async () => {
      try {
        ...
        if (auth.currentUser) {
          ...
          const savedName = localStorage.getItem('educhat_name');
          if (savedName) {
            setName(savedName);
            setJoined(true);
          }
  ...
  const handleJoin = () => {
    if (name.trim() && user) {
      localStorage.setItem('educhat_name', name.trim());
      setJoined(true);
      setError('');
    } else if (!user) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // If Firebase operation fails, store message locally
  ...
  const handleLogout = () => {
    localStorage.removeItem('educhat_name');
    setName('');
    setJoined(false);
    setMessages([]);
    setNewMessage('');
...
export default EduVerseFirebaseChat;